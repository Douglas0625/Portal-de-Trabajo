import { obtenerDatos } from "../js/api.js";
import { renderizarNavbar } from "../js/navbar.js";
...
// -------------------------
// Cargar postulantes por oferta (CORREGIDA)
// -------------------------
async function cargarPostulantesPorOferta() {
  try {
    // 1. Traer aplicaciones
    const aplicaciones = await obtenerDatos("/applications");
    const listaAplicaciones = Array.isArray(aplicaciones) ? aplicaciones : aplicaciones.data || [];

    // 2. Traer perfiles (para obtener nombres, títulos y fotos)
    const perfilesApi = await obtenerDatos("/profiles");
    const perfiles = Array.isArray(perfilesApi) ? perfilesApi : perfilesApi.data || [];

    // 3. Agrupar aplicaciones y enriquecerlas con datos del perfil
    listaAplicaciones.forEach((app) => {
      const ofertaId = app.job_post_id;

      // Buscar el perfil completo que coincida con el profile_id de la aplicación
      const perfilCompleto = perfiles.find(p => Number(p.id) === Number(app.profile_id));

      if (!allPostulantes[ofertaId]) {
        allPostulantes[ofertaId] = [];
      }

      // Guardar la aplicación "fusionada" con los datos del perfil
      allPostulantes[ofertaId].push({
        ...app,
...
// -------------------------
// Crear item postulante (CORREGIDA)
// -------------------------
// -------------------------
// Crear item postulante (CON PROTECCIÓN ANTI-ENLACES ROTOS)
// -------------------------
function crearItemPostulante(app) {
  ...
  // Formar el nombre real
  const nombreReal = `${perfil.first_name || ""} ${perfil.last_name || ""}`.trim();
  const nombreFinal = nombreReal || "Candidato Anónimo";

  // Avatar por defecto (Iniciales)
  const avatarPorDefecto = `https://ui-avatars.com/api/?name=${encodeURIComponent(nombreFinal)}&background=random`;

  // Extraer foto (si no tiene, usa el defecto)
  const fotoCandidato = perfil.profile_image_url || avatarPorDefecto;
  const tituloCandidato = perfil.professional_title || "Profesional";
      ...
      <div class="border rounded-4 p-3 d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-2">
          <img src="${fotoCandidato}" 
               ...
               class="rounded-circle flex-shrink-0" width="40" height="40" alt="${nombreFinal}" style="object-fit: cover;">
          <div class="overflow-hidden">
            <div class="fw-bold text-truncate" style="max-width: 150px;">${nombreFinal}</div>
            <small class="text-secondary text-truncate d-block" style="max-width: 150px;">${tituloCandidato}</small>
          </div>
        </div>
        <a href="#" class="fw-bold text-decoration-none ver-perfil-link flex-shrink-0" data-candidato-id="${candidatosId}" data-app-id="${app.id}" style="color:#554DEF;">Ver perfil</a>
      </div>
    </div>
  `;
}
...
function activarLinksPerfiles() {
  document.querySelectorAll(".ver-perfil-link").forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      window.location.href = `perfilDeCandidato.html?id=${candidatoId}&app=${appId}`;
    });
  });
}
...
function actualizarEstadisticas(ofertas) {
  ...
  const totalPostulantes = ofertasOriginales.reduce((sum, oferta) => {
    return sum + (allPostulantes[oferta.id] ? allPostulantes[oferta.id].length : 0);
  }, 0);
...
function aplicarFiltros() {
  ...
  const estado = selectEstado?.value.toLowerCase() || "todas";
